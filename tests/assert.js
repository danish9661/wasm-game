...
// swing (J) until food drops, then shoot (K) for a second kill.
  ...
  {
    // 30. weapon bar renders all 10 slots; a fresh NG+ run owns Fists only.
    const slots = await evaluate(`document.querySelectorAll('#weaponbar .wslot').length`);
    slots === 10 ? ok('weapon bar renders 10 slots') : bad(`weapon bar slots=${slots}`);
    const locked = await evaluate(`document.querySelectorAll('#weaponbar .wslot.locked').length`);
    locked === 9 ? ok('fresh run locks 9 weapon slots (Fists owned)') : bad(`locked slots=${locked}`);
