...
pub(crate) fn build(
    ...
    // Leg swing for a running gait
    let phase = anim_time * 6.0 + seed;
    let leg1 = (phase).sin() * 3.0;
    let leg2 = (phase + 3.14).sin() * 3.0;

    vec![
        // Lean body
        Part::vquad(cx - 7.0, cy - 11.0, 14.0, 9.0, body, alpha, true),
        // Head (forward, pointed snout)
        Part::vquad(cx + 6.0, cy - 14.0, 7.0, 7.0, body, alpha, true),
        Part::diamond(cx + 10.0, cy - 13.0, 3.0, 2.5, 0.0, light, alpha, true),
        // Pointed ears
        Part::diamond(cx + 4.0, cy - 20.0, 2.0, 3.0, 0.0, dark, alpha, true),
        Part::diamond(cx + 8.0, cy - 20.0, 2.0, 3.0, 0.0, dark, alpha, true),
        // Eyes
        Part::diamond(cx + 5.0, cy - 14.0, 1.2, 1.2, 0.0, eye, alpha, true),
        Part::diamond(cx + 9.0, cy - 14.0, 1.2, 1.2, 0.0, eye, alpha, true),
        ...
        Part::diamond(cx - 10.0, cy - 12.0, 2.0, 4.0, 0.0, dark, alpha, true),
    ]
}
