...
fn push_styled_sprite(
    ...
    use crate::elements::{
        altar, anvil, archer, arrow, banner, barrel, bat, bed, bone_pile, brazier, brute, bush, cactus, campfire, car, chest,
        crate_box, crystal, dungeon, enchanting_table, fence, fern, flower,
        grass_tuft, healing_totem, humanoid, hpbar, imp, lantern,
        lilypad, mushroom, ocean_leviathan, ore, pillar, portal, raider, rail, reed, rock, rock_pile, rubble, ruin_tower, scorpion_queen, sign,
        slime, spider, stormcaller, statue, toad_king, torch, totem, train, tree, turret, vines, wall, well, wraith, wolf,
    ...
    match style {
        ...
        SpriteStyle::Car => {
            rasterize(&car::build(cx, cy, color, alpha, facing, hw, hh), out)
        }
        SpriteStyle::Train => {
            rasterize(&train::build(cx, cy, color, alpha, facing, hw, hh), out)
        }
        SpriteStyle::Rail => {
            rasterize(&rail::build(cx, cy, color, alpha, facing, hw, hh), out)
        }
